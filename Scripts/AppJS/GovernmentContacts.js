...
$(document).ready(function () {
    ...
    const checkOrgID = urlParams.get('organizationId');
    if (checkOrgID) {
        openEditOrganizationModal(checkOrgID)
    }
          ...
          //  $('.Apploader').show();
            ...
            if (result && result.length > 0) {
                ...
                if (len > 0) {
                    ...
                    const html = '<tr orgId=' + orgId + ' class ="activities"><td colspan = 20><div>' + $('#Divactivity').html() + '</div></td></tr>';
                    $(this).addClass("shown");
...
function fetchOrganizationList(searchText = "", pageNo=0) {
    ...
    var data = searchText ? "{'keyword':'" + searchText + "', 'pageNo':" + pageNo + ",'pageSize':15}" : "{'pageNo':" + pageNo +",'pageSize':15}";
    var url = "/CrmPeople/GetOrganizationList";
